...
// Función para generar XML STB
function generarXML($user, $pass, $serial) {
    return '<?xml version="1.0" encoding="ISO-8859-1" standalone="yes"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" 
                  xmlns:cai3="http://schemas.ericsson.com/cai3g1.2/" 
                  xmlns:amx="http://schemas.ericsson.com/ma/CA/HFCTVEL/">
  <soapenv:Header>
  ...
  <soapenv:Body>
    <cai3:Delete>
      <cai3:MOType>STB@http://schemas.ericsson.com/ma/CA/HFCTVEL/</cai3:MOType>
      <cai3:MOId>
        <amx:OrderId>' . $serial . '</amx:OrderId>
      </cai3:MOId>
      <cai3:MOAttributes>
        <amx:DeleteSTB OrderId="' . $serial . '">
          <amx:UCode>UninstallSTB</amx:UCode>
          <amx:SerialNumber>' . $serial . '</amx:SerialNumber>
        </amx:DeleteSTB>
      </cai3:MOAttributes>
    </cai3:Delete>
  </soapenv:Body>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] === 0) {
        ...
        // Determinar índice del serial
        if (strtolower($primera[0]) == 'serial') {
            $index_serial = 0;
        } else {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if ($serial === '') {
                enviarMensaje('⚠️ [' . $i . '] Saltado: Serial vacío');
                fwrite($log, timestamp() . " [$i] Saltado: Serial vacío\n");
                $omitidos++;
                continue;
            }

            // Generar XML y registrarlo en el log
            $xml = generarXML($usuario, $contrasena, $serial);
            fwrite($log, timestamp() . " [$i] XML generado:\n" . $xml . "\n\n");

            // Enviar el mensaje al frontend
            enviarMensaje('➡️ [' . $i . '] Enviando: OrderId=' . $serial . ' - ' . timestamp());
